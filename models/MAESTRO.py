...
def ruler_masking(X, mask_ratio):
    """Non-Random Masking (NRM): mask a contiguous block of cells from one extreme
    of the sample's first principal component, so a phenotypically coherent
    neighbourhood is removed rather than scattered individual cells."""
    B, N, D = X.shape
    device = X.device
    ...
    # Which extreme of the PC1 ordering to mask is randomised *per sample*.
    take_high = torch.rand(B, device=device) > 0.5
    high_indices = sorted_indices[:, -N_mask:]
    low_indices = sorted_indices[:, :N_mask]
    mask_indices = torch.where(take_high.unsqueeze(1), high_indices, low_indices)
...
class SetTransformer(nn.Module):
    def __init__(self, dim_input, dim_output, num_inds=16, dim_hidden=128, dim_latent=256, num_heads=1, num_seeds=1, num_outputs=30000, ln=True):
        ...
        # Non-linear projection head (Methods 3.3). The ReLU sits *between* two linear
        # layers rather than after the output layer, so the head is non-linear while its
        # output stays unconstrained logits — a ReLU on the output itself would clamp every
        # negative logit to exactly 0, tying those dimensions together before the softmax.
    ...
    def forward_pooling(self, X):
        ...
        # Raw projection-head logits. The softmax that turns these into a probability
        # distribution is applied once, inside the self-distillation loss, together with
        # the student/teacher temperatures (see MAESTRO.calculate_kl_divergence).
...
class MAESTRO(nn.Module):
    def __init__(self, dim_input, dim_output, num_inds, dim_hidden, dim_latent, num_heads, num_outputs, ln, number_cells_subset, student_temperature, teacher_temperature, sinkhorn_start=0, teacher_beta=0.99, center_momentum=0.9):
        ...
        self.teacher_beta = teacher_beta          # EMA decay for teacher weights
        self.center_momentum = center_momentum    # EMA decay for the teacher centering vector

        # Registered as buffers so they are saved in checkpoints, restored on resume,
        # and moved with .to(device) along with the rest of the module.
        self.register_buffer('centerlatent', torch.zeros(1, dim_latent))
        self.register_buffer('current_teacher_temperature', torch.tensor(float(teacher_temperature)))
    ...
    def calculate_kl_divergence(self, student_logits, teacher_probs):
        """
        Self-distillation loss: KL( student || teacher ) 🔄

        Softmax is applied exactly once per side. The student's projection-head logits are
        turned into a distribution here at the student temperature; the teacher's
        distribution has already been centred and sharpened at the teacher temperature by
        `apply_centering_and_sharpening`.

        Parameters:
        - student_logits: Raw projection-head logits from the student model 🎓
        - teacher_probs: Centred + sharpened probability distribution from the teacher 👨‍🏫

        Returns:
        - kl_div: KL divergence value 🔄
        """
        student_log_probs = F.log_softmax(student_logits / self.student_temperature, dim=-1)
        teacher_probs = teacher_probs + 1e-9  # added for numerical stability
        kl_div = F.kl_div(student_log_probs, teacher_probs, reduction='batchmean')
        return kl_div

    def apply_centering_and_sharpening(self, teacher_logits):
        """
        Centre the teacher's projection-head logits and sharpen them into a probability
        distribution at the current teacher temperature 🎯

        Parameters:
        - teacher_logits: Raw projection-head logits from the teacher model 👨‍🏫

        Returns:
        - sharpened_output: Sharpened and centered teacher distribution 🎯
        """
        centered_output = teacher_logits - self.centerlatent.unsqueeze(0)
        sharpened_output = torch.softmax(centered_output / self.current_teacher_temperature, dim=-1)
        return sharpened_output

    @torch.no_grad()
    def _update_teacher(self, beta=None):
        """
        Update the teacher model parameters with momentum 🛠️

        Parameters:
        - beta: Momentum factor; defaults to the model's configured teacher_beta 🔄
        """
        if beta is None:
    ...
    def update_center(self, teacher_logits):
        ...
        Under distributed training the batch mean is all-reduced so every rank keeps an
        identical centre, matching the single-GPU behaviour.
        ...
        if dist.is_available() and dist.is_initialized():
            ...
            current_mean /= dist.get_world_size()
        momentum = self.center_momentum
        self.centerlatent.mul_(momentum).add_((1 - momentum) * current_mean)
    ...
    def update_temperature(self):
        ...
        """
        self.current_teacher_temperature.clamp_(max=self.student_temperature)
        if self.current_teacher_temperature.item() < self.student_temperature:
            self.current_teacher_temperature.add_(self.temperature_step).clamp_(max=self.student_temperature)
    ...
    def forward(self, X):
        ...
        # ── Teacher updates ───────────────────────────────────────────────────
        self.update_center(teacher_projection)
        teacher_projection_sharpened = self.apply_centering_and_sharpening(teacher_projection)
...
class MAESTROLightning(L.LightningModule):
    def __init__(self, dim_input, dim_output, num_inds, dim_hidden, dim_latent, num_heads, num_outputs, ln, number_cells_subset, initial_lr, min_lr, epochs, student_temperature, teacher_temperature, output_path, sinkhorn_start=0, teacher_beta=0.99, center_momentum=0.9):
        super().__init__()
        self.save_hyperparameters()
        self.model = MAESTRO(dim_input=self.hparams.dim_input,
    ...
    def on_train_epoch_end(self):
        # NOTE: this hook must run on *every* rank. The teacher temperature schedule and
        # the per-epoch metric buffers are per-rank state, so gating them behind
        # @rank_zero_only would leave non-zero ranks with a frozen teacher temperature
        # and buffers that grow without bound. Only the reporting below is rank-zero.
